...
\begin{frame}[fragile]
	\begin{center}
		\begin{tabular}{lllll}
			\toprule
			Buchstabe			&	Aussehen (klein)	&	\color{math-cmd}{Mathe}\color{black}{-Befehl}			&	Aussehen (groß)		&	\color{math-cmd}{Mathe}\color{black}{-Befehl}			\\ \midrule
	...
	\end{outputbox}
	\vspace{0.3cm}
\note[item]<1->{Erwähne VAR Versionen (varphi, varepsilon)}
\note[item]<1->{Große Versionen existieren nicht für alle}
\end{frame}
% -----------------------------------------------------------------------------------%
% ---------------------------------------FRAME---------------------------------------%
% -----------------------------------------------------------------------------------%
...
\begin{frame}[fragile]
	...
	\begin{outputbox}
		{\large\textbf{1.2.2 Die \textSC{Banach}sche- oder iterative Methode}}
	...
	\end{outputbox}
	\vspace{0.3cm}
\note[item]<1->{Text hat i.A. andere Schriftart}
\end{frame}
% -----------------------------------------------------------------------------------%
% ---------------------------------------FRAME---------------------------------------%
% -----------------------------------------------------------------------------------%
...
\begin{frame}[fragile]
	...
	\end{outputbox}
	\vspace{1.3cm}
\note[item]<1->{Schriftartmodifikation in Mathe analog zu Text}
\note[item]<1->{keine Leerzeichen in Mathe}
\end{frame}
% -----------------------------------------------------------------------------------%
% ---------------------------------------FRAME---------------------------------------%
% -----------------------------------------------------------------------------------%
...
\begin{frame}[fragile]
	...
	\begin{outputbox}
		{ \large\textbf{1.2.3 Die  \textSC{Cauchy}sche oder funktionentheoretische Methode}}
...
\begin{frame}[fragile]
	\begin{center}
		\begin{tabular}{ll}
		    ...
		    \lstinline|\quad|				&	Abstand: \num{1}$\,\mathrm{em}$		\\ 
			\lstinline|\qquad|				&	Abstand: \num{2}$\,\mathrm{em}$		\\
			\lstinline|\,|					&	Abstand: kleines Leerzeichen		\\
			\lstinline|\!|					&	Abstand: negatives kleines Leerzeichen		\\
			\bottomrule
		\end{tabular}
	\end{center}
	\pause\btVFill
	...
	\begin{outputbox}
	    \begin{align}
		    \frac{1}{2 \pi \imath} \int_C \frac{\mathrm{f}(z)}{z - \zeta} \, \mathrm{d} \zeta \tag{5}
	    \end{align}	
	\end{outputbox}
	\vspace{1.3cm}
\note[item]{The "em" is a font depending length, frequently as wide as a capital M in the current font.}
\end{frame}
% -----------------------------------------------------------------------------------%
% ---------------------------------------FRAME---------------------------------------%
% -----------------------------------------------------------------------------------%
\begin{frame}[fragile]
	...
	\begin{lstlisting}[gobble=4]
    \begin{align}
		\frac{1}{2 \pi \imath} \int_C \frac{\mathrm{f}(z)}{z - \zeta} \, \mathrm{d} \zeta
    \end{align}
	\end{lstlisting}
\note[item]<1->{$\backslash$imath}
\note[item]<1->{$\backslash$mathrm{d}}
\note[item]<1->{Abstand vor dem $\mathrm{d}$}
\end{frame}
...
\begin{frame}[fragile]
    ...
    \end{outputbox}
	\vspace{0.3cm}
\note[item]<1->{benutzt nur eine leere Zeile statt par oder zeilenumbruch, weil später ein Align eingefügt wird}
\end{frame}
% -----------------------------------------------------------------------------------%
% ---------------------------------------FRAME---------------------------------------%
% -----------------------------------------------------------------------------------%
...
\begin{frame}[fragile]
	\Aufgabee
		Erstelle folgende neue \lstinline[basicstyle=\normalfont\normalsize]|\subsubsection| über dem Abschnitt \qquote{Mathematische Methoden}:
	\begin{outputbox}
	    { \large\textbf{1.1.3 Ausnutzen des Fortpflanzungsverhaltens} } 
...
\begin{frame}[fragile]
	...
	\begin{outputbox}
	    \begin{align}
	        \frac{\mathrm{d} L}{\mathrm{d} t} \propto L \tag{3}
	    \end{align}	
    \end{outputbox}
	\vspace{1.3cm}
\end{frame}
...
\begin{frame}[fragile]
	...
	\Code
		\begin{lstlisting}[gobble=12]
		    \begin{align}
		        \frac{\mathrm{d} L}{\mathrm{d} t} \propto L 
		    \end{align}
		\end{lstlisting}
\note[item]<1->{benutze MATHRM!! für das "d"}
\end{frame}
% -----------------------------------------------------------------------------------%
% ---------------------------------------FRAME---------------------------------------%
% -----------------------------------------------------------------------------------%
\begin{frame}[fragile]
	...
	\pause\btVFill
	\Aufgabee
		Füge zwischen \qquote{$\tau$} und \qquote{darüber hinaus} folgende \lstinline[basicstyle=\normalfont\normalsize]|align|-Umgebung ein:
		\begin{outputbox}
		    \begin{align}
...
\begin{frame}[fragile]
	...
	\Code
		\begin{lstlisting}[gobble=12]
		    ...
		    \end{align}
		\end{lstlisting}
\note[item]{quad ist nicht notwendig, hilft aber zur Übersichtlichkeit}
\end{frame}
